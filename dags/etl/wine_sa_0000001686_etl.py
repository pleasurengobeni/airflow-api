...
def read_config():
    with open('dags/config/alcohol_type_sa_wine_sa_0000001686.json', 'r') as file:
...
def get_max_date(**kwargs):
    ...
    if execution_date:
        ...
        query = f"SELECT coalesce(MAX(timedimensionend), '{start_date}'::timestamp)::date FROM {table_schema}.{table_name}"
...
def calculate_end_date(start_date, filter_type):
    if start_date:
        ...
        if filter_type == 'yearly':
            return (start_date_obj + relativedelta(years=1)).date().isoformat()
        elif filter_type == 'weekly':
            return (start_date_obj + relativedelta(weeks=1)).date().isoformat()
        elif filter_type == 'daily':
            return start_date  # No change for daily, return as-is
        elif filter_type == 'monthly':
            return (start_date_obj + relativedelta(months=1)).date().isoformat()
        else:
...
def save_tracking(**kwargs):
    ...
    start_date = (parse(start_date) - timedelta(days=1))
    end_date = f"{str(calculate_end_date(start_date, config['filter']))}"
...
# Define the DAG
with DAG('ALCOHOLTYPE_SA_WINE_SA_0000001686_etl', schedule_interval=read_config()["schedule_interval"] , start_date=datetime(2024, 1, 1), catchup=False, max_active_runs=1) as dag:
    ...
    upload_to_postgres_task >> delete_file_task >> save_tracking >> end_taskub