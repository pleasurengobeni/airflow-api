...
# Append listen_addresses to postgresql.conf to allow remote connections
echo "listen_addresses = '*'" >> /var/lib/postgresql/data/postgresql.conf

# Append the connection rule to pg_hba.conf for remote connections
...
# Create the database if it doesn't exist and set privileges
...
# Create the schema for the pipeline if it doesn't exist
psql -U ${DATALAKE_DB_USER} -d ${DATALAKE_DB} -c "CREATE SCHEMA IF NOT EXISTS pipeline;"

# Create the table for tracking DAG executions if it doesn't exist
